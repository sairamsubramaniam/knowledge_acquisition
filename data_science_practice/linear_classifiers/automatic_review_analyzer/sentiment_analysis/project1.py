...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    # Your code here
    return max(0, 1 - (  label * (theta @ feature_vector.T + theta_0)  )   )
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    if feature_matrix.size==0:
        return 0
    losses = np.maximum(  1 - (  labels * (theta @ feature_matrix.T + theta_0)  ) , 0  )
...
def perceptron_single_step_update(
    ...
    """
    # Your code here
    pred = ( feature_vector @ current_theta.T + current_theta_0)
    loss = ( label * pred )
    if loss <= 0:
...
def average_perceptron(feature_matrix, labels, T):
    ...
    current_theta = np.zeros(feature_matrix.shape[1])
    current_theta_0 = 0
    final_theta = current_theta.copy()
    final_theta_0 = current_theta_0 * 1
    ...
    return final_theta / total_iters, final_theta_0 / total_iters
